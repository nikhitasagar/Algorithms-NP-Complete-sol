...
def ChooseRmVertex(sol,loss):
	...
	for i in range (k):
		...
		if loss[temp] < loss[best] and loss[temp] != 0:
			best = temp
	return best

# Initialized vertex cover
def LocalSearch1(graph,cutoff,seed):
	random.seed(seed)
	t = time()
	adjnode = dict()
	print graph
	...
	tracefile = open(graph+'_LS1_'+str(cutoff)+'_'+str(seed)+'.trace','w')
	while time() - t < cutoff:
		...
		# Find uncovered adjacent nodes of u
		mylist = [item for item in adjnode[u] if item not in c]
		# pick up one uncovered adjacent node v randomly
		v = random.choice(mylist)
		# Add v to the c
		(c,loss,uncover) = UpdateAdd(c,v,loss,uncover,adjnode)
	solfile = open(graph+'_LS1_'+str(cutoff)+'_'+str(seed)+'.sol','w')
	# print len(opt)
	# print opt
